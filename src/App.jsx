...
const T = {
  en: {
    ...
    start: "START", notStarted: "NOT STARTED", signOut: "SIGN OUT",
    ...
    generating: "Generating your personalized analysis...", docTitle: "YOUR DOCUMENT CHECKLIST",
    formTitle: "N-400 FORM WALKTHROUGH", riskTitle: "YOUR RISK ASSESSMENT", interviewTitle: "INTERVIEW PREP",
  ...
  es: {
    ...
    start: "COMENZAR", notStarted: "NO INICIADO", signOut: "CERRAR SESIÓN",
    aiDisclaimer: "Esta es solo orientación de preparación, no asesoramiento legal.",
    generating: "Generando tu análisis personalizado...", docTitle: "TU LISTA DE DOCUMENTOS",
    formTitle: "GUÍA DEL FORMULARIO N-400", riskTitle: "TU EVALUACIÓN DE RIESGOS", interviewTitle: "PREPARACIÓN PARA ENTREVISTA",
  ...
  zh: {
    ...
    start: "开始", notStarted: "未开始", signOut: "退出登录",
    aiDisclaimer: "这仅是准备指导，不是法律建议。",
    generating: "正在生成您的个性化分析...", docTitle: "您的文件清单",
    formTitle: "N-400表格说明", riskTitle: "您的风险评估", interviewTitle: "面试准备",
  ...
  tl: {
    ...
    sendLink: "MAGPADALA NG MAGIC LINK", checkEmail: "SURIIN ANG IYONG EMAIL", checkEmailSub: "Naipadala na ang iyong magic link. I-click ang link sa email mo.",
    ...
    start: "SIMULAN", notStarted: "HINDI PA SINIMULAN", signOut: "MAG-SIGN OUT",
    aiDisclaimer: "Ito ay gabay sa paghahanda lamang, hindi legal na payo.",
    generating: "Ginagawa ang iyong personalisadong pagsusuri...", docTitle: "ANG IYONG LISTAHAN NG DOKUMENTO",
    formTitle: "GABAY SA FORM N-400", riskTitle: "ANG IYONG PAGTATASA NG PANGANIB", interviewTitle: "PAGHAHANDA SA PANAYAM",
  ...
  vi: {
    ...
    start: "BẮT ĐẦU", notStarted: "CHƯA BẮT ĐẦU", signOut: "ĐĂNG XUẤT",
    aiDisclaimer: "Đây chỉ là hướng dẫn chuẩn bị, không phải tư vấn pháp lý.",
    generating: "Đang tạo phân tích cá nhân hóa của bạn...", docTitle: "DANH SÁCH TÀI LIỆU CỦA BẠN",
    formTitle: "HƯỚNG DẪN MẪU ĐƠN N-400", riskTitle: "ĐÁNH GIÁ RỦI RO CỦA BẠN", interviewTitle: "CHUẨN BỊ PHỎNG VẤN",
  ...
  hi: {
    ...
    sub: "आपके N-400 के लिए AI-संचालित चरण-दर-चरण तैयारी। कोई वकील नहीं चाहिए। एकमुश्त $49।",
    ...
    q3: "क्या आप एक बार में 6 महीने से अधिक समय के लिए अमेरिका से बाहर गए हैं?",
    ...
    modDesc: ["जानें कि क्या इकट्ठा करना है।", "हर सवाल को समझें।", "USCIS से पहले अपने जोखिम जानें।", "जब तक आत्मविश्वास न आए तब तक अभ्यास करें।"],
    start: "शुरू करें", notStarted: "शुरू नहीं हुआ", signOut: "साइन आउट",
    aiDisclaimer: "यह केवल तैयारी मार्गदर्शन है, कानूनी सलाह नहीं।",
    generating: "आपका व्यक्तिगत विश्लेषण तैयार हो रहा है...", docTitle: "आपकी दस्तावेज़ चेकलिस्ट",
    formTitle: "N-400 फॉर्म वॉकथ्रू", riskTitle: "आपका जोखिम मूल्यांकन", interviewTitle: "साक्षात्कार तैयारी",
  ...
  pt: {
    ...
    start: "INICIAR", notStarted: "NÃO INICIADO", signOut: "SAIR",
    aiDisclaimer: "Isto é apenas orientação de preparação, não assessoria jurídica.",
    generating: "Gerando sua análise personalizada...", docTitle: "SUA LISTA DE DOCUMENTOS",
    formTitle: "GUIA DO FORMULÁRIO N-400", riskTitle: "SUA AVALIAÇÃO DE RISCOS", interviewTitle: "PREPARAÇÃO PARA ENTREVISTA",
...
const N400_SECTIONS = [
  ...
  { section: "Part 3 — Residence & Employment", questions: [
    { id: "3a", q: "Where have you lived in the last 5 years?", explain: "List every address where you lived for more than 3 months in the past 5 years.", example: "123 Main St, Miami FL (2020–present)", mistake: "Forgetting a short-term address or leaving gaps in your timeline." },
  ...
  { section: "Part 4 — Travel Outside the U.S.", questions: [
    { id: "4a", q: "Have you traveled outside the U.S. in the last 5 years?", explain: "List every trip outside the U.S. even if just for a few days.", example: "Mexico, June 1–June 10 2023, 9 days", mistake: "Forgetting short weekend trips or trips to Canada." },
    { id: "4b", q: "Have you ever been absent from the U.S. for more than 6 months?", explain: "A single trip over 6 months can break your continuous residence. This is critical.", example: "Yes, I was in Brazil from Jan 2019 to Aug 2019.", mistake: "Not disclosing this — USCIS will find it in travel records." },
  ]},
...
export default function App() {
  ...
  const handleSendMagicLink = async () => { if (!email) return; await supabase.auth.signInWithOtp({ email, options: { emailRedirectTo: "https://legaliai.com" } }); setMagicSent(true); };
  ...
  const handleModuleOpen = (idx) => { const names = ["documents","form","risk","interview"]; setPage(names[idx]); if (idx===0&&!aiContent.documents) generateDocuments(); if (idx===2&&!aiContent.risk) generateRisk(); };
  ...
  const generateDocuments = async () => {
    setLoadingAI(true);
    try {
      const text = await callAI(`You are an immigration preparation assistant. Based on these N-400 answers: ${JSON.stringify(onboardingAnswers)}, generate a personalized document checklist. Return ONLY a JSON array: [{name, source, reason}]. Respond in ${langName}.`);
      setAiContent(prev => ({ ...prev, documents: JSON.parse(text.replace(/```json|```/g,"").trim()) }));
    } catch {
      setAiContent(prev => ({ ...prev, documents: [{name:"Permanent Resident Card",source:"Your physical card",reason:"Proves immigration status"},{name:"Passport from birth country",source:"Home country embassy",reason:"Confirms identity"},{name:"Tax returns (last 5 years)",source:"IRS or tax preparer",reason:"Proves U.S. residence"},{name:"Travel records",source:"Passport stamps",reason:"Documents all trips outside U.S."},{name:"Marriage certificate (if applicable)",source:"Vital records office",reason:"Required if married to U.S. citizen"}] }));
    }
    setLoadingAI(false);
  };
  ...
  const generateRisk = async () => {
    setLoadingAI(true);
    try {
      const text = await callAI(`Based on these N-400 answers: ${JSON.stringify(onboardingAnswers)}, identify 3-5 risk factors. Return ONLY a JSON array: [{title, explanation, action}]. Respond in ${langName}. Be specific and simple.`);
      setAiContent(prev => ({ ...prev, risk: JSON.parse(text.replace(/```json|```/g,"").trim()) }));
    } catch {
      setAiContent(prev => ({ ...prev, risk: [{title:"Continuous Residence",explanation:"Any trip over 6 months could interrupt your continuous residence.",action:"Gather all travel records and explain long trips."},{title:"Good Moral Character",explanation:"USCIS reviews the past 5 years of your conduct.",action:"Disclose all interactions with law enforcement."}] }));
    }
    setLoadingAI(false);
  };

  const handleFormFeedback = async (qId, question, answer) => {
    const feedback = await callAI(`Review N-400 answer. Question: "${question}" Answer: "${answer}". Give 2-3 sentence feedback. Any red flags? Respond in ${langName}.`);
    setFormFeedback(prev => ({ ...prev, [qId]: feedback }));
  };

  const handleInterviewSubmit = async (idx, question, answer) => {
    const feedback = await callAI(`Evaluate N-400 interview answer. Question: "${question}" Answer: "${answer}". Rate 1-10, one strength, one tip. 3 sentences. Be encouraging. Respond in ${langName}.`);
    ...
    return (
      <div style={{minHeight:"100vh",background:"#000",fontFamily:"'Cormorant Garamond',serif",paddingTop:"80px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"calc(100vh - 80px)",padding:"40px"}}>
          ...
          {q.type==="yesno"&&<div style={{display:"flex",gap:"16px",marginBottom:"32px"}}>
          ...
          {q.type==="choice"&&<div style={{display:"flex",flexDirection:"column",gap:"12px",marginBottom:"32px",width:"100%",maxWidth:"500px"}}>
            {q.opts.map((opt,i)=>(
          ...
          {t("modules").map((mod,i)=>(
            <div key={i} style={{background:"#0a0a0a",border:"1px solid #1a1a1a",padding:"40px",display:"flex",flexDirection:"column",gap:"16px"}}>
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:"auto",paddingTop:"16px"}}>
                <span style={{color:"#C9A84C",fontSize:"10px",letterSpacing:"2px"}}>{t("notStarted")}</span>
